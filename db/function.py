...
async def _write_position(writer: InfluxWriter, position: Dict) -> None:
    ...
    try:
        ...
        logging.error(f"Error writing position data for {position.get('asset', 'Unknown')}: {e}")
        raise
...
def read_from_influx(data_type: str = 'latest_positions', 
    ...
    try:
        ...
        logging.error(f"{Fore.RED}Error reading from InfluxDB: {e}{Style.RESET_ALL}")
        return []
    finally:
        if reader:
            reader.close()

def _validate_query_params(data_type: str, asset: Optional[str], hours: int) -> bool:
    ...
    if data_type not in valid_types:
        logging.error(f"Invalid data type: {data_type}. Must be one of {valid_types}")
        return False
...
def _format_output(data_type: str, data: List[Dict], asset: Optional[str] = None) -> None:
...
def _print_position_data(positions: List[Dict]) -> None:
    ...
    sorted_positions = sorted(positions, key=lambda x: x.get('total_notional', 0), reverse=True)
    ...
    avg_ls_ratio = sum(p.get('ls_ratio', 0) for p in positions) / len(positions)
    print(f"""
...
def _print_history_point(point: Dict) -> None:
    ...
    change_str = f"{change_color}{price_change:+.2f}%{Style.RESET_ALL}" if price_change is not None else "N/A"
...
def _print_metric_entry(metric: Dict) -> None:
    ...
    change_str = f"{change_color}{volume_change:+.2f}%{Style.RESET_ALL}" if volume_change is not None else "N/A"
